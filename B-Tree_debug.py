...
## DELETE
    ...
    def balancing_child(self,pnode,pos,cpos):
        print("Balancing Child ", pnode , cpos , pnode.child[cpos])
        if cpos==0:# 맨 왼쪽 자식
            ...
            if self.distribute_with_left(pnode,cpos):
                ...
                print("Merge") 
                self.merge(pnode,pos,cpos-1,cpos)
                pass
            ## Merge
        else:
            print("중간에 껴있는 자식")
    ...
    def distribute_with_left(self, pnode,pos):
        if len(pnode.child[pos-1].keys) >= self.m//2 + 1: # 왼쪽으로부터 가져오기 가능
            pnode.child[pos].keys = pnode.keys[pos] + pnode.child[pos].keys
            pnode.keys[pos] = pnode.child[pos-1].key[-1]
...
# bt.root,'\n',bt.root.child , bt.root.child[0].child
bt.delete(bt.root,[20,22])
# bt.delete(bt.root,[60,0])